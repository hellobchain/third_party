...
func New(defaultAlgo string, req *csr.CertificateRequest) (cert, csrPEM, key []byte, err error) {
	policy := CAPolicy()
	if req.CA != nil {
		if req.CA.Expiry != "" {
	...
	csrPEM, key, err = g.ProcessRequest(defaultAlgo, req)
	if err != nil {
...
func RenewFromSigner(ca *x509.Certificate, priv crypto.Signer) ([]byte, error) {
	...
	req := csr.ExtractCertificateRequest(defaultAlgo, ca)
