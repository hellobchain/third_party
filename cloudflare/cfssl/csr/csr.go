...
func NewBasicKeyRequest(algo string) *BasicKeyRequest {
	return &BasicKeyRequest{algo, curveP256}
}
...
// BasicKeyRequest.
func New(algo string) *CertificateRequest {
	return &CertificateRequest{
		KeyRequest: NewBasicKeyRequest(algo),
	}
}
...
func ParseRequest(defaultAlgo string, req *CertificateRequest) (csr, key []byte, err error) {
	log.Info("received CSR")
	if req.KeyRequest == nil {
		req.KeyRequest = NewBasicKeyRequest(defaultAlgo)
	}
...
func ExtractCertificateRequest(defaultAlgo string, cert *x509.Certificate) *CertificateRequest {
	req := New(defaultAlgo)
...
func (g *Generator) ProcessRequest(defaultAlgo string, req *CertificateRequest) (csr, key []byte, err error) {
	...
	csr, key, err = ParseRequest(defaultAlgo, req)
	if err != nil {
		return nil, nil, err
	}
	return
