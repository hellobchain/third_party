...
var hashOIDs = map[newcryptosm.Hash]asn1.ObjectIdentifier{
	newcryptosm.SHA1:   asn1.ObjectIdentifier([]int{1, 3, 14, 3, 2, 26}),
	newcryptosm.SHA256: asn1.ObjectIdentifier([]int{2, 16, 840, 1, 101, 3, 4, 2, 1}),
	newcryptosm.SHA384: asn1.ObjectIdentifier([]int{2, 16, 840, 1, 101, 3, 4, 2, 2}),
	newcryptosm.SHA512: asn1.ObjectIdentifier([]int{2, 16, 840, 1, 101, 3, 4, 2, 3}),
	newcryptosm.SM3:    asn1.ObjectIdentifier([]int{1, 2, 156, 10197, 1, 401, 1}),
}
...
var signatureAlgorithmDetails = []struct {
	...
	{x509.MD2WithRSA, oidSignatureMD2WithRSA, x509.RSA, newcryptosm.Hash(0) /* no value for MD2 */},
	{x509.MD5WithRSA, oidSignatureMD5WithRSA, x509.RSA, newcryptosm.MD5},
	{x509.SHA1WithRSA, oidSignatureSHA1WithRSA, x509.RSA, newcryptosm.SHA1},
	{x509.SHA256WithRSA, oidSignatureSHA256WithRSA, x509.RSA, newcryptosm.SHA256},
	{x509.SHA384WithRSA, oidSignatureSHA384WithRSA, x509.RSA, newcryptosm.SHA384},
	{x509.SHA512WithRSA, oidSignatureSHA512WithRSA, x509.RSA, newcryptosm.SHA512},
	{x509.DSAWithSHA1, oidSignatureDSAWithSHA1, x509.DSA, newcryptosm.SHA1},
	{x509.DSAWithSHA256, oidSignatureDSAWithSHA256, x509.DSA, newcryptosm.SHA256},
	{x509.ECDSAWithSHA1, oidSignatureECDSAWithSHA1, x509.ECDSA, newcryptosm.SHA1},
	{x509.ECDSAWithSHA256, oidSignatureECDSAWithSHA256, x509.ECDSA, newcryptosm.SHA256},
	{x509.ECDSAWithSHA384, oidSignatureECDSAWithSHA384, x509.ECDSA, newcryptosm.SHA384},
	{x509.ECDSAWithSHA512, oidSignatureECDSAWithSHA512, x509.ECDSA, newcryptosm.SHA512},
	{x509.SM2WithSM3, oidSignatureSM2WithSM3, x509.SM2, newcryptosm.SM3},
	{x509.SM2WithSHA1, oidSignatureSM2WithSHA1, x509.SM2, newcryptosm.SHA1},
	{x509.SM2WithSHA256, oidSignatureSM2WithSHA256, x509.SM2, newcryptosm.SHA256},
}

// TODO(rlb): This is also from crypto/x509, so same comment as AGL's below
func signingParamsForPublicKey(pub interface{}, requestedSigAlgo x509.SignatureAlgorithm) (hashFunc newcryptosm.Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	var pubType x509.PublicKeyAlgorithm

	switch pub := pub.(type) {
	case *rsa.PublicKey:
		pubType = x509.RSA
		hashFunc = newcryptosm.SHA256
	...
	case *ecdsa.PublicKey:
		...
		case elliptic.P384():
			hashFunc = newcryptosm.SHA384
			sigAlgo.Algorithm = oidSignatureECDSAWithSHA384
		case elliptic.P521():
			hashFunc = newcryptosm.SHA512
			sigAlgo.Algorithm = oidSignatureECDSAWithSHA512
		case sm2.SM2():
			hashFunc = newcryptosm.SM3
...
func getHashAlgorithmFromOID(target asn1.ObjectIdentifier) newcryptosm.Hash {
	for hash, oid := range hashOIDs {
		if oid.Equal(target) {
			return hash
		}
	}
	return newcryptosm.Hash(0)
}

func getOIDFromHashAlgorithm(target newcryptosm.Hash) asn1.ObjectIdentifier {
	for hash, oid := range hashOIDs {
		if hash == target {
			return oid
		}
...
type Request struct {
	HashAlgorithm  newcryptosm.Hash
	IssuerNameHash []byte
	IssuerKeyHash  []byte
	SerialNumber   *big.Int
}
...
func ParseRequest(bytes []byte) (*Request, error) {
	...
	if hashFunc == newcryptosm.Hash(0) {
...
func (opts *RequestOptions) hash() newcryptosm.Hash {
	if opts == nil || opts.Hash == 0 {
		// SHA-1 is nearly universally used in OCSP.
		return newcryptosm.SHA1
	}
	return opts.Hash
}
...
func CreateResponse(issuer, responderCert *x509.Certificate, template Response, priv crypto.Signer) ([]byte, error) {
	...
	if template.IssuerHash == 0 {
		template.IssuerHash = newcryptosm.SHA1
	}
