...
func NewWithParams(securityLevel int, hashFamily string, keyStore bccsp.KeyStore) (bccsp.BCCSP, error) {
	...
	// Set the Encryptors
	swbccsp.AddWrapper(reflect.TypeOf(&symmetryPrivateKey{}), &aescbcpkcs7Encryptor{})

	// Set the Decryptors
	swbccsp.AddWrapper(reflect.TypeOf(&symmetryPrivateKey{}), &aescbcpkcs7Decryptor{})
	...
	swbccsp.AddWrapper(reflect.TypeOf(&bccsp.SM2KeyGenOpts{}), &sm2KeyGenerator{curve: sm2.SM2()})
	swbccsp.AddWrapper(reflect.TypeOf(&bccsp.AESKeyGenOpts{}), &symmetryKeyGenerator{length: conf.aesBitLength})
	swbccsp.AddWrapper(reflect.TypeOf(&bccsp.AES256KeyGenOpts{}), &symmetryKeyGenerator{length: 32})
	swbccsp.AddWrapper(reflect.TypeOf(&bccsp.AES192KeyGenOpts{}), &symmetryKeyGenerator{length: 24})
	swbccsp.AddWrapper(reflect.TypeOf(&bccsp.AES128KeyGenOpts{}), &symmetryKeyGenerator{length: 16})
	swbccsp.AddWrapper(reflect.TypeOf(&bccsp.SM4KeyGenOpts{}), &symmetryKeyGenerator{length: conf.sm4BitLength})
	...
	swbccsp.AddWrapper(reflect.TypeOf(&symmetryPrivateKey{}), &symmetryPrivateKeyKeyDeriver{conf: conf})
