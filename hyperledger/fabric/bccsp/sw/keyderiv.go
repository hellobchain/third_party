...
type symmetryPrivateKeyKeyDeriver struct {
	conf *config
}

func (kd *symmetryPrivateKeyKeyDeriver) KeyDeriv(k bccsp.Key, opts bccsp.KeyDerivOpts) (bccsp.Key, error) {
	// Validate opts
	if opts == nil {
		return nil, errors.New("Invalid opts parameter. It must not be nil.")
	}

	aesK := k.(*symmetryPrivateKey)
	...
	case *bccsp.HMACTruncated256AESDeriveKeyOpts:
		...
		return &symmetryPrivateKey{mac.Sum(nil)[:kd.conf.aesBitLength], false}, nil

	case *bccsp.HMACDeriveKeyOpts:
		mac := hmac.New(kd.conf.hashFunction, aesK.privKey)
		mac.Write(hmacOpts.Argument())
		return &symmetryPrivateKey{mac.Sum(nil), true}, nil
