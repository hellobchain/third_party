...
import (
	...
	"github.com/wsw365904/third_party/hyperledger/fabric-config/configtx/orderer"
	"math"
	"reflect"
	"strings"
	"time"
...
func (o *OrdererGroup) GetConsenter(host string) (orderer.Consenter, error) {
	cfg, err := o.Configuration()
	if err != nil {
		return orderer.Consenter{}, err
	}

	if cfg.OrdererType != orderer.ConsensusTypeEtcdRaft {
		return orderer.Consenter{}, fmt.Errorf("consensus type %s is not etcdraft", cfg.OrdererType)
	}

	for _, c := range cfg.EtcdRaft.Consenters {
		if strings.ContainsAny(c.Address.Host, host) {
			return c, nil
		}
	}
	return orderer.Consenter{}, fmt.Errorf("no find Consenter")
}
